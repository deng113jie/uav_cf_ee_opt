...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        mu = mu_std[:, :, 0]
        mu = torch.nan_to_num(mu, nan=0.5, posinf=0.0, neginf=0.0)
        log_std = mu_std[:, :, 1].clamp(-20, 2)
    ...
    def sample(self, state):
        ...
        # log_prob = normal.log_prob(z) - torch.log(1 - action.pow(2) + 1e-6 + torch.log(torch.tensor(1))  )  # for tanh
        log_prob = torch.log(action) + torch.log(1 - action + 1e-6)  # method 1
        log_prob += torch.log(torch.tensor(1))  # Only if you scale actions
        log_prob = normal.log_prob(z) - log_prob
        log_prob = log_prob.sum(dim=-1, keepdim=True)  # Sum over action dimensions
        # log_prob = normal.log_prob(z) - torch.log(action * (1 - action) + 1e-6)  # method 2
...
class SAC:
    def __init__(self, state_dim, action_dim):
        ...
        self.alpha = 0.5
        self.target_entropy = -0.5*action_dim  # action range 0 to 1
    ...
    def act(self, state):
        ...
        return action.squeeze(0).cpu().data.numpy()
    ...
    def learn(self):
        ...
        dones = torch.tensor(np.array(dones)).float().unsqueeze(1)
        ...
        # print('Loss ', critic_loss, actor_loss, alpha_loss)
    ...
    def compute_ap_contributions(self, state, action, global_reward):
        ...
        action_tensor = torch.tensor(action).float().unsqueeze(0).requires_grad_(True)
        # SAC uses two critics - compute both Q-values
        q1, q2  = self.critic(state_tensor, action_tensor)
        # Use minimum Q-value for attribution (aligns with SAC policy updates)
        q_min = torch.min(q1, q2)
        # Zero gradients to prevent accumulation
        self.critic.zero_grad()
        # Backpropagate from the minimum Q-value
        q_min.backward(retain_graph=False)
        contributions = torch.abs(action_tensor.grad).squeeze()
        contributions = contributions / (torch.sum(contributions) + 1e-8)
        return (contributions * global_reward).cpu().data.numpy()

# Modified Training Loop with Curriculum Learning
def train(curriculum=False):
