function [gainOverNoisedB,R,pilotIndex,D,APpositions,UEpositions,distances,mu_path_gain] = generateSetup(L,K,N,tau_p,nbrOfSetups,aplayout)
...
for n = 1:nbrOfSetups
    ...
    if aplayout == 'kmeans'
        if L>K
            disp("running extended kmean to place aps...")
            APpositions = repmat(UEpositions,[1, floor(L/K)]);
            APpositions = reshape(APpositions,[],1);
            if rem(L, K) ~= 0
                temp = functionAPpositions(rem(L,K), UEpositions);
                APpositions(size(APpositions,1)+1:size(APpositions,1)+size(temp,1)) = temp;
            end
            assert(size(APpositions,1) == L)
        else
            disp("running kmean to place aps...")
            APpositions = functionAPpositions(L, UEpositions);
        end
    else
